...
#### Arbitrary-signature WASM support

- `WasmGenericMultivector` and `WasmGenericRotor` — single WASM-bindgen types for all
  Clifford algebra signatures Cl(p, q, r) with runtime signature selection
- Match-table dispatch for all 84 (p, q, r) combinations with DIM ≤ 6 (compile-time
  optimized, generated by build script)
- Cayley-table fallback for DIM > 6 with thread-local caching using
  `amari-core::generic`
- 8 fast-path aliases: Cl(3,0,0), Cl(2,1,0), Cl(3,1,0), Cl(2,0,0), Cl(0,3,0),
  Cl(4,1,0), Cl(5,0,0), Cl(1,1,0)
- Single TypeScript `Multivector` class with `(p, q, r)` constructor and 8 factory
  methods
- Convenience exports: `GA`, `ST`, `MINK`, `PL`, `QUAT`, `CGA`, `P5D`, `S2D`
- Generic `BatchOperations` and `PerformanceOperations` accept `(p, q, r)` parameters

#### `amari-core`

- `blade_product(p, q, r, dim, i, j)` — computes result blade index and sign for
  the geometric product of two basis blades at runtime
- `generic_geometric_product(p, q, r, a, b)` — computes full geometric product from
  coefficient slices for any signature
- Both functions support positive (+1), negative (-1), and null (0) metric types

### Changed

- `amari-wasm`: replaced per-signature `WasmMultivector`/`WasmSpacetimeMultivector`
  concrete types with `WasmGenericMultivector` + 8 fast-path aliases
- `amari-wasm`: `BatchOperations` and `PerformanceOperations` now accept generic
  `(p, q, r)` parameters instead of being hardcoded to Cl(3,0,0)
- TypeScript: merged per-signature `Multivector`, `SpacetimeAlgebra` classes into a
  single `Multivector` class with `(p, q, r)` constructor
- `WasmMultivector` and `WasmRotor` remain as type aliases for backward compatibility
